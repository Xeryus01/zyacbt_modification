...
class Modul_soal extends Member_Controller
{
	...
	public function index($id_soal = null)
	{
		...
		if ($id_soal != null) {
			...
			if ($query_soal->num_rows() > 0) {
				...
				$data['data_paket'] = $query_soal->soal_paket_id;
	...
	function paket()
	{
		...
		$paket = $this->cbt_paket->get_by_kolom('topik_id', $topik_id)->result();
		$i = 0;
		foreach ($paket as $pack) {
			$result[$i] = $pack->paket_id;
			$i++;
		}
	...
	function tambah()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$id_paket = $this->input->post('tambah-paket-id', TRUE);
	...
	function get_by_id($id = null)
	{
		...
		if (!empty($id)) {
			...
			if ($query->num_rows() > 0) {
				...
				$data['id_paket'] = $query->soal_paket_id;
			}
		}
	...
	function upload_file()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$id_paket = $this->input->post('image-paket-id', true);
			$posisi = $this->config->item('upload_path') . '/topik_' . $id_topik;
	...
	function get_datatable()
	{
		...
		$query = $this->cbt_soal_model->get_datatable_paket($start, $rows, 'soal_detail', $search, $topik, $paket);
		$iFilteredTotal = $query->num_rows();

		$iTotal = $this->cbt_soal_model->get_datatable_count_paket('soal_detail', $search, $topik, $paket)->row()->hasil;
		...
		foreach ($query as $temp) {
			...
			if (!empty($temp->soal_audio)) {
				$posisi = $this->config->item('upload_path') . '/topik_' . $temp->soal_topik_id . '_' . $temp->soal_paket_id;
				$soal = $soal . '<br />
					<audio controls>
