...
class Modul_topik extends Member_Controller
{
	...
	function tambah()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if ($this->cbt_topik_model->count_by_topik_modul($data['topik_nama'], $data['topik_modul_id'])->row()->hasil > 0) {
				 ...
				 * SAVE FIRST PAKET
				 */
				$topik = $this->cbt_topik_model->get_by_kolom('topik_nama', $data['topik_nama']);
				$topik = $topik->row();
				$paket['topik_id'] = $topik->topik_id;
				$paket['paket_id'] = 1;
				$this->cbt_paket->save($paket);
	...
	function get_datatable()
	{
		...
		foreach ($query as $temp) {
			...
			$paket = $this->cbt_paket->get_by_kolom('topik_id', $temp->topik_id)->result();
			...
			foreach ($paket as $pack) {
				$jml = $this->cbt_soal_model->count_per_paket($temp->topik_id, $pack->paket_id)->row()->hasil;
				$jml_soal .= $jml . "<br>";

				$paket_i++;
				$package .= "Paket " . $pack->paket_id . "\n";
			}
			...
			$record[] = '<a onclick="paket(\'add\', \'' . $temp->topik_id . '\', \'' . $paket_i . '\')" style="cursor: pointer;" class="btn btn-default btn-xs">tambah paket</a>
			<a onclick="paket(\'delete\', \'' . $temp->topik_id . '\', \'' . $paket_i . '\')" style="cursor: pointer;" class="btn btn-default btn-xs">hapus paket</a>';
	...
	function paket()
	{
		$pilihan = $this->input->post('paket_pilihan', true);
		$topik_id = $this->input->post('paket_topik-id', true);
		$paket_id = $this->input->post('paket_paket-id', true);
		if ($pilihan == 'delete') { //delete paket
			$this->cbt_paket->delete($topik_id, $paket_id);
		} else if ($pilihan == 'add') { //tambah paket
			$paket['topik_id'] = $topik_id;
			$paket['paket_id'] = $paket_id + 1;
			$this->cbt_paket->save($paket);
		}
		redirect(base_url() . 'index.php/manager/modul_topik/');
	}
