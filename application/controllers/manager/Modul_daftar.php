...
class Modul_daftar extends Member_Controller
{
	...
	function count()
	{
		...
		$result = $this->cbt_soal_model->count_per_paket($topik, $paket)->row()->hasil;

		echo json_encode($result);
	}

	function cetak_soal($topik_id = null, $paket_id = null)
	{
		if (!empty($topik_id)) {
			$query_soal = $this->cbt_soal_model->get_cetak($topik_id, $paket_id);
